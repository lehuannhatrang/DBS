	...
	foreach ($pages as $pi) {
		...
		if (is_array($pi['alias'])) {
			if ($lang && isset($pi['alias'][$lang])) {
				return (!$mod_rewrite && !preg_match('#^\.\.\/\?route=#', $pi['alias'][$lang]) ? '../?route=' : '').
						(strpos($pi['alias'][$lang], '#') === false ? $lang : '').
						(rtrim($pi['alias'][$lang], '/') ? '/' : '').rtrim($pi['alias'][$lang], '/').'/';
			} else if ($def_lang && isset($pi['alias'][$def_lang])) {
				return (!$mod_rewrite && !preg_match('#^\.\.\/\?route=#', $pi['alias'][$def_lang]) ? '../?route=' : '').
						(strpos($pi['alias'][$lang], '#') === false ? $def_lang : '').
						(rtrim($pi['alias'][$def_lang], '/') ? '/' : '').rtrim($pi['alias'][$def_lang], '/').'/';
			}
		} else {
			return (!$mod_rewrite && !preg_match('#^\.\.\/\?route=#', $pi['alias']) ? '../?route=' : '').
					'/'.rtrim($pi['alias'], '/').'/';
		}
	}
}
